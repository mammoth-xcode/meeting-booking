...
const AddRoomTypeModal = ({ isOpen, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    if (validate()) {
      ...
      try {
        ...
        // POST request to add a new roomtype
        ...
        if (response.ok) {
          ...
          onSave(updatedData);  // Call the onSave function to update the roomtype list
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white  px-6 py-4  rounded-lg w-96">
        <div className="container mx-auto px-0 mb-1 flex items-center justify-center">
          ...
          <span className="text-lg font-semibold mb-0">เพิ่มประเภทห้องประชุมใหม่</span>
        </div>
        ...
        {loading && (
          <div className="flex flex-row items-center justify-center container mx-auto">
            <span className="mr-3 text-slate-800 text-base">กำลังเพิ่มประเภทห้องประชุม</span>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-[9999]">
            <div className="bg-white p-4 rounded-lg w-72 flex justify-center items-center">
              <h3 className="text-green-700 text-lg font-semibold text-center">
                เพิ่มประเภทห้องประชุมเรียบร้อยแล้ว
              </h3>
            </div>
          </div>
        )}
        ...
        {showUnKnown && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white p-4 rounded-lg w-72 flex justify-center items-center">
              <h3 className="text-red-700 text-lg font-semibold text-center">
                มีปัญหาในการเพิ่มข้อมูลประเภทห้องประชุม !
              </h3>
            </div>
          </div>
        )}
