...
import { FaBook, FaCircleExclamation, FaAddressBook, FaUserPlus, FaUserPen, FaFloppyDisk, FaArrowRotateLeft, FaCirclePlus, FaHouseUser, FaCube } from "react-icons/fa6";
...
const AddEquipmentModal = ({ isOpen, onClose, onSave }) => {
  ...
  const validate = () => {
    ...
    if (!formData.equipment_name) {
      newErrors.equipment_name = "กรุณากรอกชื่ออุปกรณ์";
      valid = false;
    } else {
  ...
  const handleSave = async () => {
    if (validate()) {
      ...
      try {
        ...
        // POST request to add a new equipment
        ...
        if (response.ok) {
          ...
          onSave(updatedData);  // Call the onSave function to update the equipment list
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white  px-6 py-4  rounded-lg w-96">
        <div className="container mx-auto px-0 mb-1 flex items-center justify-center">
          <FaCube className="text-2xl font-semibold mb-1 px-0.5 " />
          <span className="text-lg font-semibold mb-0">เพิ่มอุปกรณ์ใหม่</span>
        </div>
        <div className="space-y-4">
          {/* equipment name Input */}
          <Label className="text-slate-700">ชื่ออุปกรณ์</Label>
          <div>
            <Input
              className="-mt-3 mb-1"
              label="ชื่ออุปกรณ์"
              ...
              disabled={loading}
            />
            {errors.equipment_name && <p className="text-red-500 text-sm">{errors.equipment_name}</p>}
          </div>
          
        </div>
        {loading && (
          <div className="flex flex-row items-center justify-center container mx-auto">
            <span className="mr-3 text-slate-800 text-base">กำลังเพิ่มอุปกรณ์</span>
        ...
        {showUnKnown && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white p-4 rounded-lg w-72 flex justify-center items-center">
              <h3 className="text-red-700 text-lg font-semibold text-center">
                มีปัญหาในการเพิ่มข้อมูลอุปกรณ์ !
              </h3>
            </div>
          </div>
        )}
