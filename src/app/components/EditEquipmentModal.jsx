...
const EditEquipmentModal = ({ isOpen, equipment, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    if (validate()) {
        // Ensure that equipment are saved with their IDs
        const updatedData = {
            ...formData,
        };
        ...
        try {
          ...
          // Check if user and equipment_id are valid
          if (!equipment || !equipment.equipment_id) {
              alert("equipment ID is missing");
              return;
          }
          
          const response = await fetch(`api/equipments/${equipment.equipment_id}`, {
              method: "PUT",
              headers: {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white  px-6 py-4  rounded-lg w-96">
        <div className="container mx-auto px-0 mb-1 flex items-center justify-center">
          ...
          <span className="text-lg font-semibold mb-0">แก้ไขข้อมูลอุปกรณ์</span>
        </div>
        <div className="space-y-4">
          {/* equipment ID Input */}
          <Label className="text-slate-700">รหัสอุปกรณ์</Label>
          <div>
            <Input
              className="-mt-3 mb-1"
              label="รหัสอุปกรณ์"
